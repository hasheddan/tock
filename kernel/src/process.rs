...
pub trait ProcessType {
    ...
    fn set_syscall_return_value(&self, return_value: GenericSyscallReturnValue);
...
impl<C: Chip> ProcessType for Process<'_, C> {
    ...
    fn set_syscall_return_value(&self, return_value: GenericSyscallReturnValue) {
        match self.stored_state.map(|stored_state|
            // Actually set the return value for a particular process. The UKB
            // implementation is passed the bounds of process-accessible memory
            // so it can verify that any memory changes are valid. This is
            // encapsulated in an unsafe block here because we are guaranteeing
